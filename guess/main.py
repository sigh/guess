...
def main():
    ...
    if len(sys.argv) > 1 and sys.argv[1] in subcommands:
        ...
        subparsers = parser.add_subparsers(dest='converter_type', help='Force specific converter type')
        
        # Time/timestamp subcommand
        time_parser = subparsers.add_parser('time', help='Force timestamp interpretation')
        time_parser.add_argument('value', help='Value to convert as timestamp')
        
        # Duration subcommand
        duration_parser = subparsers.add_parser('duration', help='Force duration interpretation')
        duration_parser.add_argument('value', help='Value to convert as duration')
        
        # Size/byte size subcommand
        size_parser = subparsers.add_parser('size', help='Force byte size interpretation')
        size_parser.add_argument('value', help='Value to convert as byte size')
        
        # Number subcommand
        number_parser = subparsers.add_parser('number', help='Force number base interpretation')
        ...
        parser.add_argument(
            "value",
            nargs="?",
            help="Value to convert (auto-detect type)"
        )
